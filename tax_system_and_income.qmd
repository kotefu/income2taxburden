...
```{r deduct_basic}
deduct_basic <- function(income_earned, resid_or_income) {
  b1 <- 2400
  b2 <- 2450
  b3 <- 2500
  
  ## 所得税・住民税に応じて、引数で変えられるようにした
  if (resid_or_income == "income") {
    
    ## 所得税の控除額を返す場合
    ## 国民民主党の案を反映できるよう、koku項を追加
    if (income_earned <= b1) {
      48 + koku
...
```{r calc_pension}
calc_pension <- function(income_earned, nat_or_emp) {
  unit <- 1e4
...
## 引数の単位は万円。
## 引数numは世帯の人数
calc_kokuho <- function(income_earned, num, city_table, is_salary) {

  ## 被用者の場合に給与所得控除を差し引く
  if (is_salary == TRUE) {
    income <- income_earned - deduct_income(income_earned)
  } else if (is_salary == FALSE) {
    income <- income_earned
  }

  ## 課税標準所得額 (所得 - 住民税型基礎控除) をincomeとする
  income <- income - deduct_basic(income_earned, "resid")
...
## 年収が430万円、一人世帯の場合
## このとき、課税所得が300万円になる
calc_kokuho(200, 1, saitama, FALSE)
```
...
calc_kenpo <- function(income_earned, pref) {
  income_month <- income_earned / 12 # 年収を月当たりに
  unit <- 1e4
  data <- kenpo_table |> 
    filter(
...
deduct_insur <- function(
  ...
  ## 年金保険料
  pension <- calc_pension(income_earned, nat_or_emp)
  
  ## 国民健康保険料
  kokuho <- calc_kokuho(income_earned, num, city_table, is_salary)
  
  ## 協会けんぽ保険料
  kenpo <- calc_kenpo(income_earned, pref)
...
```{r}
deduct_insur(200, 1, saitama, "埼玉", FALSE, "nat")
deduct_insur(200, 1, saitama, "埼玉", TRUE, "emp")
```
...
get_income_taxable <- function(
  ...
  ## 控除
  ded_insurance <- deduct_insur(income_earned, num, city_table, pref = "埼玉", is_salary, nat_or_emp)
  ded_basic <- deduct_basic(income_earned, resid_or_income)

  ## earned incomeから、控除額を差し引く = taxable income
  ## 今回のモデルでは、入口はincome_earnedだけなので、
  ## income_taxableが合計所得金額＝総所得金額等になる
  income_taxable <- income_earned - sum(ded_basic, ded_insurance)
...
```{r}
get_income_taxable(200, 1, saitama, "nat", TRUE, "resid")
```
...
## numは、扶養家族人数
calc_tax_income <- function(income_taxable, num = 1) {
...
```{r}
get_income_taxable(200, 1, saitama, "nat", FALSE, "income") |> 
  calc_tax_income(1)
```
...
```{r}
get_income_taxable(200, 1, saitama, "nat", FALSE, "resid") |> 
  calc_tax_resident(0.1, 1)
...
```{r}
calc_vat <- function(income_earned, num = 1, city = saitama, nat_or_emp, is_salary) {
  ## earned incomeごとの税・社会保険料の合計値を得たい
  ## 所得税
  tax_income <- get_income_taxable(
    income_earned, num, city, nat_or_emp, is_salary, "income"
    ) |> 
    calc_tax_income(num)
  ## 住民税
  tax_resit <- get_income_taxable(
    income_earned, num, city, nat_or_emp, is_salary, "resid"
    ) |> 
    calc_tax_resident(tax_rate = 0.1, num)
  
  ## 健康保険
  if (nat_or_emp == "nat") {
    insurance <- calc_kokuho(income_earned, num, city, is_salary)
  } else if (nat_or_emp == "emp") {
    insurance <- calc_kenpo(income_earned, "埼玉")
  }
  ...
  consume <- income_earned - total # 消費額
...
```{r}
calc_vat(200, 1, saitama, "nat", FALSE)
```
...
```{r calc_tax_and_prem}
calc_tax_and_prem <- function(income_earned, num, city_table, is_salary, nat_or_emp) {
  
  print(paste("Earned income:", income_earned))
  
  ## 課税所得額
  taxable_incometax <- get_income_taxable(income_earned, num, city_table, nat_or_emp, is_salary, "income")
  taxable_residtax <- get_income_taxable(income_earned, num, city_table, nat_or_emp, is_salary, "resid")
  
  ## 年金・健康保険
  if (nat_or_emp == "nat") {
    ## 自営業者
    pension <- calc_pension(income_earned, nat_or_emp)
    insurance <- calc_kokuho(income_earned, num, city_table, is_salary)
  ...
  ## 住民税
  resident <- taxable_residtax |> calc_tax_resident(tax_rate = 0.1, num)
  print(paste0("住民税: ", resident))

  ## 所得税
  income_tax <- taxable_incometax |> calc_tax_income(num)
  print(paste("所得税:", income_tax))
  
  ## 消費税
  vat <- calc_vat(income_earned, num, city_table, nat_or_emp, is_salary)
  print(paste("消費税:", vat))
...
```{r}
result_gig <- calc_tax_and_prem(200, 1, saitama, FALSE, "nat")
result_gig
```
...
result <- make_fig_gig(result_gig)
result
```


```{r}
calc_tax_and_prem(200, 1, saitama, TRUE, "emp")
```

```{r}
calc_tax_and_prem(150, 1, saitama, TRUE, "nat")
```
